...
/**
 * ForwardAuth extension
 *
 * @author  Eweol <eweol@outlook.com>
 * @package SeedDMS
 * @subpackage  forward_auth
 */
class SeedDMS_ForwardAuth extends SeedDMS_ExtBase
{
	...
	function init()
	{
		$GLOBALS['SEEDDMS_HOOKS']['initDMS'][] = new SeedDMS_ForwardAuth_initDMS;
	}
...
class SeedDMS_ForwardAuth_initDMS
{
	...
	function postInitDMS($array)
	{
		...
		$dms = $array['dms'];
		$forwardAuthSettings = $extSettings['forward_auth'];

		$headers = getRequestHeaders();

		if (!isset($forwardAuthSettings['forward_authEnable'])) {
			return;
		}

		if ($forwardAuthSettings['forward_authEnable'] !== "1") {
			return;
		}
		...
		$username = $headers[strtolower($forwardAuthSettings['usernameHeader'])];
